...
export default class Newspagecomp extends Component<AppProps, AppState> {
  ...
  componentDidMount() {
    fetch("http://localhost:8000/backend/newsblog/getnews/1/", {
      ...
      .catch(error => {
        console.log("Did not get images");
      });
  }
  ...
  render() {
    return (
      <div>
        <h1
          style={{ paddingLeft: "40px", paddingTop: "40px", color: "#03205c" }}
        >
          NEWS
        </h1>
        <Container
                  ...
                  <CardContent>
                    ...
                    >
                      ...
                      <p style={{ fontSize: "10px" }}>
                        Published on:{" "}
                        {obj.date_uploaded.replace("T", " ").slice(0, 19)}
                      </p>
                    </Container>
                    <ReactQuill
                      value={JSON.parse(obj.newsops)}
                      modules={{ table: true, tableUI: true }}
                      readOnly={true}
                    />
                  </CardContent>
                </Card>
